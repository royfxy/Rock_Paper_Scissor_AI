...
def parse_args():
    ...
    parser.add_argument('--run', type=str, default='game',
                        help='run game or train model')
    parser.add_argument('--model_name', type=str,
                        default='model.pth', help='model path')
    parser.add_argument('--gesture', type=str,
                        default='paper', help='gesture to capture')
...
if __name__ == '__main__':
    args = parse_args()
    mode = args.run
    prediction_model_pth = "rock_paper_scissor/network/gesture_prediction/checkpoints/" + \
        args.model_name + ".pth"
    recognition_model_pth = "rock_paper_scissor/network/gesture_recognition/checkpoints/" + \
        args.model_name + ".pth"
    if mode == 'game':
        ...
        if args.k_fold == 1:
            ...
            k_fold_cross_validation_train(
                args.k_fold, prediction_model_pth, args.epochs)
