...
def train(model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        data, target = data.to(device), target.type(
...
def k_fold_cross_validation_train(folds, file_pth, epochs=100):
    ...
    for fold, (train_ids, test_ids) in enumerate(k_folds.split(dataset)):
        ...
        test_loaders.append(DataLoader(
            dataset, batch_size=16, sampler=test_sampler))
    best_accuracy = 0
    ...
    for epoch in range(1, epochs + 1):
        ...
        for fold, _ in enumerate(k_folds.split(dataset)):
            train_accuracy, train_loss = train(
                models[fold], device, train_loaders[fold], optimizers[fold], epoch)
            train_accuracies.append(train_accuracy)
            train_losses.append(train_loss)
            test_accuracy, test_loss = test(
                models[fold], device, test_loaders[fold])
            test_accuracies.append(test_accuracy)
            test_losses.append(test_loss)
        print(
            f"Epoch {epoch} accuracy: {sum(test_accuracies)/len(test_accuracies)}")
        writer.add_scalar("Accuracy/train",
                          sum(train_accuracies)/len(train_accuracies), epoch)
        writer.add_scalar(
            "Accuracy/test", sum(test_accuracies)/len(test_accuracies), epoch)
        writer.add_scalar("Loss/train", sum(train_losses) /
                          len(train_losses), epoch)
        writer.add_scalar("Loss/test", sum(test_losses) /
                          len(test_losses), epoch)
