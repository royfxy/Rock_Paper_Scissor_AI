...
def train_model_static(recognition_model_pth, epoch_num=200):
    ...
    for epoch in tqdm(range(epoch_num),ascii=True):
        ...
        val_loss,val_acc = get_validation_loss(model, valid_dataloader,valid_dataloader)
        ...
        if epoch + 1 > min_epochs and val_loss < min_val_loss:
            ...
            tqdm.write('Best model at Epoch {:03d} train_loss {:.5f} val_loss {:.5f} val_acc {:.5f}'.format(epoch, np.mean(np.array(epoch_train_loss)), val_loss,val_acc))
